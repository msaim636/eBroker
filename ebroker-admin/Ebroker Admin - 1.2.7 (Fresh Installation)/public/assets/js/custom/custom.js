...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initializeTinyMCE() {
        ...
        Object.keys(allLanguages).forEach(languageId => {
            const selector = `#translation-description-${languageId}`;
            const element = document.querySelector(selector);
            
            if (element) {
                const language = allLanguages[languageId];
    ...
    setTimeout(initializeTinyMCE, 200);
});
        ...
        if (param[0]) {
            ...
            if (param[0].type_of_parameter == "radiobutton") {
                ...
                $.each(param[0].type_values, function (k, v) {
                    $('#' + param[0].id).append(
                        '<input name="par_' + param[0].id + '" type="radio" value="' + v.value + '" class="form-check-input ml-5" '+isRequired+'/>' + v + '  '
                    );
                });
            }
            if (param[0].type_of_parameter == "checkbox") {
                ...
                $.each(param[0].type_values, function (k, v) {
                    $('#' + param[0].id).append(
                        '<input name="par_' + param[0].id + '[]" type="checkbox" value="' + v.value + '" class="form-check-input" '+isRequired+'/>' + v.value + '  '
                    );
                });
            }
