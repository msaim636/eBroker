...
class RemoveCustomers extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $customerData = Customer::whereNotIn('id', $getCustomerId)->where(function($query) use ($excludeCustomerNumber) {
                $query->where(function($query) use ($excludeCustomerNumber) {
                    $query->whereNotIn('mobile', $excludeCustomerNumber)->where('logintype', '1')->where(function($query) {
                        $query->where('country_code', '91')->orWhere('country_code', '+91');
                    });
                })->orWhereNull('mobile');
