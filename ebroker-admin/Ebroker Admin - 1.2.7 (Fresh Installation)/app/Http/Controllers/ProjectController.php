...
class ProjectController extends Controller
{
    ...
    public function create(){
        ...
        return view('project.create', compact('category', 'currency_symbol', 'languages'));
    }
    ...
    public function edit($id){
        ...
        $project = Projects::where('id',$id)->with('plans:id,title,project_id,document')->with('translations')->first();
        $project['gallary_images'] = $project->gallary_images_directly->get();
        $project['documents'] = $project->documents_directly->get();
        $category = Category::where('status', '1')->get();
        $languages = HelperService::getActiveLanguages();
        return view('project.edit',compact('project','category','languages'));
    }
    ...
    public function updateRequestStatus(Request $request)
    {
        ...
        try {
            ...
            if (!has_permissions('update', 'project')) {
                ...
                $msg = "";
                if (!empty($fcm_ids)) {
                    $msg = $project->request_status == 'approved' ? 'Approved by Administrator ' : 'Rejected by Administrator ';
                    $registrationIDs = $fcm_ids[0];
