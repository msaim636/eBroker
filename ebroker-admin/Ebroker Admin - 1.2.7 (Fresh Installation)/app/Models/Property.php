...
class Property extends Model
{
    ...
    public function getParametersAttribute(){
            ...
            foreach ($parameterQueryData as $res) {
                    ...
                    if (is_string($res['pivot']['value']) && is_array(json_decode($res['pivot']['value'], true))) {
                        ...
                        foreach($value as $key => $val){
                            if($res->translated_option_value){
                                foreach($res->translated_option_value as $key => $translatedVal){
                                    if($translatedVal['value'] == $val){
                                        $translatedValue[] = $translatedVal['translated'];
                                    }
                                }
                            }
                        }
                    ...
                    if(collect($value)->isNotEmpty()){
                        $parameters[] = [
                            ...
                            'translations' => $res->translations->map(function($translation){
                                return [
		...
		// Sort parameters following the category's parameter_types order
		try {
			$category = $this->relationLoaded('category:id,parameter_types') ? $this->category : $this->category()->select('id','parameter_types')->first();
			if ($category && !empty($category->parameter_types) && !empty($parameters)) {
				$orderIds = array_values(array_filter(array_map('intval', explode(',', $category->parameter_types))));
				usort($parameters, function ($a, $b) use ($orderIds) {
					$posA = array_search((int)$a['id'], $orderIds);
					$posB = array_search((int)$b['id'], $orderIds);
            ...
            foreach ($assignFacilitiesQuery as $facility) {
                if(collect($facility->outdoorfacilities)->isNotEmpty()){
                    $assignFacilitiesData[] = [
                        ...
                        'translations' => $facility->outdoorfacilities->translations->map(function($translation){
                            return [
                                'language_id' => $translation->language_id,
                                'value' => $translation->value,
                            ];
