...
sealed class Filter {
  ...
  static T empty<T extends Filter>() {
    return switch (T) {
      const (PropertyTypeFilter) => PropertyTypeFilter.empty() as T,
      const (PostedSince) => PostedSince.empty() as T,
      const (MinMaxBudget) => MinMaxBudget.empty() as T,
      const (CategoryFilter) => CategoryFilter.empty() as T,
      const (LocationFilter) => LocationFilter.empty() as T,
      const (FacilitiesFilter) => FacilitiesFilter.empty() as T,
      const (NearbyPlacesFilter) => NearbyPlacesFilter.empty() as T,
      _ => throw ArgumentError('Unknown filter type: $T'),
    };
  }
}
